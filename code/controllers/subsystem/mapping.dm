...
/datum/controller/subsystem/mapping/Initialize(timeofday)
	...
	while (space_levels_so_far < config.space_ruin_levels)
		++space_levels_so_far
		add_new_zlevel("Empty Area [space_levels_so_far]", ZTRAITS_SPACE, overmap_obj = new /datum/overmap_object/ruins(SSovermap.main_system, rand(1,20), rand(1,20)))
...
/datum/controller/subsystem/mapping/proc/LoadGroup(list/errorList, name, path, files, list/traits, list/default_traits, silent = FALSE, datum/overmap_object/ov_obj = null) //SKYRAT EDIT CHANGE
	. = list()
...
/datum/controller/subsystem/mapping/proc/loadWorld()
	...
	LoadGroup(FailedZs, "Station", config.map_path, config.map_file, config.traits, ZTRAITS_STATION, ov_obj = new /datum/overmap_object/station(SSovermap.main_system, rand(1,20), rand(1,20)))
	...
	if(config.minetype != "none")
		...
		if(!mining_map_to_load)
			...
			if(config.minetype == "lavaland")
				LoadGroup(FailedZs, "Lavaland", "map_files/Mining", "Lavaland.dmm", default_traits = ZTRAITS_LAVALAND,  ov_obj = new /datum/overmap_object/lavaland(SSovermap.main_system, rand(1,20), rand(1,20))) //SKYRAT EDIT CHANGE
